...
describe('BetweenInterruptionsSelectionStrategyService', () => {
	...
	describe('when only a starting date is selected and a date after it is hovered', () => {
		...
		it('but before the next interruption', () => {
			...
			castDraft(store.state()).calendar.interruptions = [new Interruption('', Interval.after(interruptionStart, {week: 1}).toFullDay())];
		...
		describe('and after the next interruption', () => {
			...
			it('the interruption being ignored', () => {
				service.ignoredInterruptions = castDraft(store.state()).calendar.interruptions;
	...
	describe('when a range is selected', () => {
		...
		it('should drag the start until the previous interruption', () => {
			...
			castDraft(store.state()).calendar.interruptions = [new Interruption('', Interval.before(interruptionEnd, {week: 1}).toFullDay())];
